...
| `free_running_gap_audit.py` | **Tier-0 free-running pathology probe**: teacher-forced vs greedy free-running accuracy over the generation horizon (read A — TF acc by distance-from-block-start; read B — TF−free-running gap by horizon, per content/structural tier) → verdict `healthy`/`exposure_bias`/`short_context_or_bug`; `--verify-cache` recomputes each free-run from scratch to rule out a KV-cache/position bug. Gates the remediation ladder in `design/generation/free_running_pathology_remediation_design.md`. | `test_free_running_gap_audit.py` |
| `copy_novel_audit.py` | **Copy-vs-novel decomposition**: splits teacher-forced accuracy by whether the target is induction-copyable from in-window history vs novel (per tier) — model-conditioned "is the accuracy real or just copying" (the static pathology smoking gun). | `test_copy_novel_audit.py` |
| `effective_context_audit.py` | **Effective-context probe**: TF accuracy vs truncated context length k; the saturation point is the model's real context horizon (settles the gap probe's `short_context` branch). | `test_effective_context_audit.py` |
| `event_position_audit.py` | **Position-within-event accuracy**: buckets TF accuracy by atom role (voice-tag/kind/digit/nibble/…) and offset-since-marker — separates the by-design front-loaded sawtooth (P4.2) from real pathology. | `test_event_position_audit.py` |
| `voice_interleave_audit.py` | **Per-voice interleave accuracy**: labels each atom's voice (event-grammar walk) and buckets TF accuracy by voice + interleave gap — localizes content failure to the cross-voice multiplex (arms the `lane_demux_hypothesis.md` / Tier-2 trigger). | `test_voice_interleave_audit.py` |
| `calibration_audit.py` | **Calibration probe**: ECE + confidence-vs-accuracy gap + predictive-entropy on content positions — overconfidence predicts sampling mode-collapse (informs the Tier-1 sampling regime). | `test_calibration_audit.py` |
| `memorization_audit.py` | **Memorization audit**: n-gram index over training blocks → novel-fraction (n=8/16) + longest verbatim-match bucket for a generated stream (torch-free; realizes the `generation_quality_gate.md` memorization check). | `test_memorization_audit.py` |
...
| `learnability_triage.py` | **training-free** ranking of a tokenization/ordering by information-theoretic learnability proxies (entropy-rate vs memory h_k per-token+per-frame, MI-decay, induction-copy rate) over the real `parse`d atom stream across configs (baseline/full_macros/codebook/skeleton + custom `label=flagA+flagB`) — substitutes for direction-finding mini A/Bs; predicts sign+ordering, not the scale threshold. Backing: `design/references/learnability_token_ordering_theory.md`. | `test_learnability_triage.py` |
| `residual_set_census.py` | **corpus acceptance gate** for the residual-SET-elimination work order: parallel, digi-excluded, progress-logged sweep that counts surviving raw `SET`s (unmodeled driver mechanisms) under the residual macro arm; VERDICT `residual_SETs=0 dirty_tunes=0` = done. Flags-drop-if-absent so it runs pre/post the residual PRs; flags an outlier tune as a suspected missed (PWM) digi. Driven by `design/RUNBOOK_residual_set_release.md` Phase D. | (image-only gate; mirrors tokens `residual_mechanism.py`) |
