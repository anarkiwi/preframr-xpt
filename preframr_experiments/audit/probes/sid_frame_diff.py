...
for v, (flo, fhi, plo, phi, ctrl, ad, sr) in VOICE.items():
    CLASS[flo] = f"v{v}.FREQ_LO"
    CLASS[fhi] = f"v{v}.FREQ_HI"
    CLASS[plo] = f"v{v}.PW_LO"
    CLASS[phi] = f"v{v}.PW_HI"
    CLASS[ctrl] = f"v{v}.CTRL"
    CLASS[ad] = f"v{v}.AD"
    CLASS[sr] = f"v{v}.SR"
...
EXACT_REGS = [
    r
    for r, c in CLASS.items()
    if (".CTRL" in c or ".AD" in c or ".SR" in c or c in ("RES_FILT", "MODE_VOL"))
]
...
def pipeline_frame_state(dump_path, cfg):
    ...
    xdf = next(
        RegLogParser(args=args).parse(
            dump_path, max_perm=1, require_pq=False, reparse=True
        ),
        None,
    )
...
def build_args(cfg):
    ...
    if cfg == "skel":
        return parse_args(
            skeleton_pass=True, trajectory_anchor_pass=True, held_arp=True
        )
    if cfg == "base":
...
def report(ref, test, cfg):
    ...
    for r in range(25):
        ...
        if r in FREQ_REGS:
            big = int(
                (
                    np.abs(ref[skip:, r].astype(int) - test[skip:, r].astype(int)) > 4
                ).sum()
            )
            flag = f"  freq Δ>4 in {big} frames" + (
                "  <<< GARBAGE" if big > n // 10 else ""
            )
    ...
    for label, regs in (
        (
            "CTRL (gate/wave)",
            [r for r in range(25) if CLASS.get(r, "").endswith("CTRL")],
        ),
        (
            "ADSR",
            [
                r
                for r in range(25)
                if CLASS.get(r, "")[-3:] in (").AD", "AD", "SR")
                and ("AD" in CLASS.get(r, "") or "SR" in CLASS.get(r, ""))
            ],
        ),
        ("FREQ", FREQ_REGS),
    ...
    print(
        "\nVERDICT:",
        (
            "FAIL — discrete (gate/ADSR) registers diverge: "
            + ", ".join(f"{c}({n_})" for _, c, n_ in bad_exact)
            if bad_exact
...
def main():
    dump = (
        sys.argv[1]
        if len(sys.argv) > 1
        else "/corpus/hvsc/MUSICIANS/J/Jammer/Grid_Runner.1.dump.parquet"
    )
