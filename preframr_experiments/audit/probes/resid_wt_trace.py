...
from preframr_tokens.stfconstants import (  # noqa: E402
    ORN_OP, WAVETABLE_DEF_OP, WAVETABLE_STEP_OP, WAVETABLE_END_OP, WAVETABLE_REF_OP,
    SKEL_OP, FREQ_TRAJ_REGS,
)
...
OPNAME = {ORN_OP: "ORN", WAVETABLE_DEF_OP: "WT_DEF", WAVETABLE_STEP_OP: "WT_STEP",
          WAVETABLE_END_OP: "WT_END", WAVETABLE_REF_OP: "WT_REF", SKEL_OP: "SKEL"}
...
def dump_rows(df, label, lo, hi):
    ...
    for i in range(max(0, lo), min(len(df), hi)):
        ...
        print(f"  {i:6d} op={nm:7s} reg={int(reg[i]):3d} sub={int(sub[i]):3d} val={int(val[i])}")
...
def main():
    ...
    op = d_on["op"].to_numpy()
    wt = np.where(np.isin(op, [WAVETABLE_DEF_OP, WAVETABLE_STEP_OP, WAVETABLE_END_OP, WAVETABLE_REF_OP]))[0]
    if len(wt):
