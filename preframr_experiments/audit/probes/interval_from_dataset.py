...
for (song, voice), freqs in seqs.items():
    ...
    abs_c.update(coll)
    int_c.update(ivals)
    per_voice[voice][0].update(coll)
    per_voice[voice][1].update(ivals)
...
print(
    f"songs={len(songs_seen)-1}  (song,voice) lines={len(seqs)}  "
...
for name, c in (("ABSOLUTE semitone", abs_c), ("INTERVAL (relative)", int_c)):
    ...
    print(
        f"  {name:<22} N={N:>7} distinct={k:>4} entropy={H:>5.2f}b  "
...
for v in sorted(per_voice):
    ...
    if sum(ac.values()) < 50:
        continue
    print(
        f"    voice {v}: abs {stat(ac)[2]:.2f} (eff {stat(ac)[3]:.0f}) "
