...
from preframr_tokens.stfconstants import (  # noqa: E402
    ...
    WAVETABLE_DEF_OP, WAVETABLE_REF_OP, WT_REF_SUBREG_ID, FREQ_TRAJ_REGS,
)
...
def analyze(args):
    paths, wid = args
    dmap = _DMAP
    drain = defaultdict(lambda: {"resid": 0, "drained": 0, "tunes": 0, "bad": 0, "verified": 0})
    ...
    for p in paths:
        ...
        if done % PROGRESS_EVERY == 0:
            ...
            print(f"[w{wid}] {done}/{len(paths)} resid={tot}", file=sys.stderr, flush=True)
...
def render(drain, tail, examples, sample_n):
    ...
    for e in sorted(drain, key=lambda e: -drain[e]["resid"]):
        ...
        lines.append(f"{e:30s} {d['tunes']:>6} {d['resid']:>7} {d['drained']:>7} {dr:>5}% "
                     f"{d['verified']:>5} {d['bad']:>4}  {cells}")
        gr += d["resid"]; gd += d["drained"]; gbad += d["bad"]; gver += d["verified"]
    gdenom = gr + gd
...
def main():
    ...
    print(f"surveying {len(sample)} tunes on {procs} procs -> {out}", file=sys.stderr, flush=True)
    ...
    with mp.Pool(min(procs, n_chunks)) as pool:
        for ci, res in enumerate(pool.imap_unordered(analyze, shards), 1):
            ...
            print(f"[parent] chunk {ci}/{n_chunks} tunes={n_done_tunes}/{len(sample)} "
    ...
    with open(out, "w") as f:
        f.write(report + "\n")
        f.write("\n# raw per-engine json:\n# " + json.dumps(
            {e: dict(d) for e, d in drain.items()}) + "\n")
    print(report)
