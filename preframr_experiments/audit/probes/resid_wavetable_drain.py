...
def main():
    ...
    print(
        f"{'engine':28s} {'RESID_off':>9} {'RESID_on':>8} {'drain%':>6} "
    ...
    for engine in ENGINES:
        ...
        tag = f"{exact_ok}/{exact_ok + exact_bad} OK" + (
            "" if not exact_bad else "  CORRUPT!"
        )
        print(
            f"{engine:28s} {off_tot:9d} {on_tot:8d} {drain:5d}% "
