...
**Learnability framing.** A REFUTED model-side content head — it founders on the ~0.13 ceiling that tokenizer-side `full_macros` then lifted. The lever is representation learnability, not the head ([`learnability_token_ordering_theory.md`](../references/learnability_token_ordering_theory.md)).
