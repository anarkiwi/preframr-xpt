...
|---|---|---|---|
| **preframr-tokens** | **PyPI wheel** (`preframr_tokens`) | the SID decompiler codec: `recover_from_sid` (sid-only recovery, via the deterministic `preframr-sidtrace` binary → `.sidwr.bin` + `.bus.bin`; generic fitter recovers the program from the bus trace), per-driver backends (hubbard/goattracker/lft, DMC in progress), the BACC primitive (subsumes VIB/SLIDE/ARP/PWM/ADSR/sweeps), the **FLAT v2 model-facing alphabet** (`flat_serialize.py`: typed atoms + `GEN_*`, no place value, no numeric LZ; **VOCAB=576**), `program_to_ids`/`measure`/`ids_to_program` (GoatTracker → flat v2 shipped, generic path's flat port in flight), `Corpus`, `constrained_decode`, `render_play`, tokenizer profiling | **torch-free** → small, fast-iterating library; **anything about token/program *semantics* lives with the codec that defines them** |
