...
|---|---|---|---|
| **preframr-tokens** | **PyPI wheel** (`preframr_tokens`) | the BACC step/tracker codec: `recover_from_sid` (sid-only recovery, via the deterministic `preframr-sidtrace` binary → `.sidwr.bin` + `.bus.bin`; generic fitter recovers the program from the bus trace), per-driver backends (hubbard/goattracker/lft, DMC in progress), the BACC primitive (subsumes VIB/SLIDE/ARP/PWM/ADSR/sweeps), `program_to_ids`/`measure`/`ids_to_program` (VOCAB=34), `Corpus`, `constrained_decode`, `render_play`, tokenizer profiling | **torch-free** → small, fast-iterating library; **anything about token/program *semantics* lives with the codec that defines them** |
