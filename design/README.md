...
| [`macro_learnability_risk_review.md`](macro_learnability_risk_review.md) | Grounded audit of the macro *implementations* (stable + in-flight residual-SET PRs) against the learnability basis. A first-cut HIGH (unbounded codebook DEF→REF distance vs mid-song prompts) was **WITHDRAWN** — the model trains/predicts on **self-contained blocks** (expand-to-literal → slice → re-encode; `block_refire_pass_names` re-fires every reference-op producer per block; `self_contained_prompt_df` + constrained-decode B3 snapshot), so DEFs are always in-block with their REFs. Surfaced a real **tool caveat**: the triage must tokenize blocks, not full songs. **MEDIUM (remain):** `note_off` ships Option B not duration (A); cross-voice frame multiplex raises the per-voice horizon. **Confirmed-good:** `ctrl_osc`/`sweep` counter-eliminating; codebook ids pure (block-local) ordinals, id-snap copy-collapse RESOLVED. | **Review 2026-06-03 (corrected)** |
| [`role_lane_factorization.md`](role_lane_factorization.md) | Reframes voice-lanes: a SID voice is an allocated **resource**, a musical **role** (melody/bass/harmony/percussion) is the model target — and roles hop voices dynamically, so a physical voice-lane splits one line and welds two roles. Abstraction = **role lanes** (within-role continuity, the transferable structure) + an **orchestration map** (role→voice over time; byte-exact-invertible) + a **voice-wiring graph** = the per-span "patch": filter/timbre bus (global filter + reg-23 routing bits) AND **sync/ring modulation edges**. **Hard constraint:** sync (ctrl bit 1) / ring (bit 2) wire voices in a fixed ring (osc N ← osc N−1), so voice N's audio depends on voice N−1's freq. **Modulation is a wiring *edge*, not a role** — the common case is *dual* (a voice plays its own line AND its oscillator feeds the next voice's sync/ring); the silent-only source is the degenerate case (load-bearing, must not be dropped). The edge references the source's existing line-freq and is encoded as the source↔carrier *relationship* (not two absolute freqs → non-local arithmetic). Ladder: **percussion first** (noise tag), **wiring graph second** (filter + sync/ring, hardware-explicit), **melody/bass inference last + GATED** (after the wiring claims silent modulators). Measure-before-build via the triage. | **Draft / design resolution 2026-06-03** |
