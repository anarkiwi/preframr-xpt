...
| [`sequence_order_normalization_design.md`](sequence_order_normalization_design.md) | Sequence-level sibling of the above: collapse the inaudible **write-order** DoF within a frame. CPU probe shows engines share structural token TYPES (cosine 1.0) + content vocab (0.96) but diverge on per-frame write ORDER (+0.169 over the order-free SET) — concentrated in VOICE_REG placement + value-latch reg pairs, re-introduced by post-sort passes (`reglogparser:909→915`). Gate-anchored canonical re-order pass. | Draft 2026-05-27 |
