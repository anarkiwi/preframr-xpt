...
| [`role_lane_factorization.md`](role_lane_factorization.md) | Reframes voice-lanes: a SID voice is an allocated **resource**, a musical **role** (melody/bass/harmony/percussion) is the model target — and roles hop voices dynamically, so a physical voice-lane splits one line and welds two roles. Abstraction = **role lanes** (within-role continuity, the transferable structure) + an **orchestration map** (role→voice over time; makes it byte-exact-invertible) + a **filter/timbre bus** (global filter + reg-23 routing bits as one channel bound to voices, not a voice property). Tractability ladder: **percussion lane first** (noise-waveform tag, mechanical, de-dilutes the pitched target), **filter bus second** (routing explicit), **melody/bass role inference last + GATED** (latent/lossy). Measure-before-build via the triage (role-lane vs voice-lane MI concentration; percussion-removal h_k drop). Cross-voice de-mux is the lever `melody_channel_factorization` pointed to (within-voice refuted). | **Draft / design resolution 2026-06-03** |
