...
| Doc | Summary | Status |
|---|---|---|
| [`trajectory_anchoring.md`](trajectory_anchoring.md) | The FREQ_TRAJ/PW/filter encoding segments trajectories on value-runs, discarding the gate/sweep anchor → melody reaches the model as un-anchored noise (FREQ_TRAJ ~0.026 acc; the full_macros content win is SET scaffolding, not melody). Two-pass intrinsic anchor detector validated across 4 tunes (incl. non-tracker, off-gate sweeps); impl spec landed in preframr-tokens. | **LANDED + mini A/B DONE**: content win (content-tier 0.036→0.080, val_acc 0.113→0.137) but **SET-carried (op0 0.063→0.175); op45/melody flat (~0.002)** — mini too weak to test melody (op45 ~0 both arms). **Prodlike A/B queued** (only regime with op45 signal) |
