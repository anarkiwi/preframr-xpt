...
**Melody learnability is the explicit NEXT layer** (the generator pipeline de-ornaments but leaves the melody
line as absolute pitch ≈ unlearnable): [`melody_skeleton_impl.md`](melody_skeleton_impl.md) re-keys freq
note-onsets to **key-invariant intervals** (the measured-transferable melody lever, 0.52 held-out > 0.41
ceiling) — a separate work order, BLOCKED on the generator pipeline landing.
