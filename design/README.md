...
| [`macro_learnability_risk_review.md`](macro_learnability_risk_review.md) | Grounded audit of the macro *implementations* (stable + in-flight residual-SET PRs) against the learnability basis. **HIGH:** every codebook (`wavetable`/`stamp`/`patch`/`ctrl_wavetable`) emits its DEF **once at first occurrence with unbounded distance to later REFs** — and the headline task is mid-song prompting, whose window excludes the song-head DEFs, so REFs become inference-unresolvable; fix = sliding-window DEF re-emit; **measure the >2048 DEF→REF fraction first**. **MEDIUM:** `note_off` ships Option B (standalone token) not duration (A) — off-event determinant is non-local; cross-voice frame multiplex raises the per-voice horizon. **Confirmed-good:** `ctrl_osc`/`sweep` are counter-eliminating; codebook ids are pure ordinals so the old id-snap copy-collapse is RESOLVED. | **Review 2026-06-03** |
