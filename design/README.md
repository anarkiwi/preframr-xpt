...
| [`universal_multiresolution_pitch.md`](universal_multiresolution_pitch.md) | **The learnable+lossless+universal pitch model** (supersedes per-tune-LUT / content-tier-residual). Shared NOTE INDEX (semitone grid, note 49≈C5 everywhere — the universal prediction target) + **per-voice recovered note→freq TABLE** (the exact tracker entries, ~20/voice, a small codebook). Static notes are PURE (**83% of frames**, validated); residual is nonzero ONLY for genuine modulation (~17%, as SWEEP/TRI/TABLE trajectories). Chorus = shared index, distinct per-voice tables (Cauldron II 75% pure). Corrected from the discarded fixed-grid+sub/LSB framing (which spuriously showed 70% modulation). Foundation landed (`pitch_grid.py` + tests). Default-OFF, byte-exact gated. | **Active — validated (83% pure on real corpus); wiring next** |
