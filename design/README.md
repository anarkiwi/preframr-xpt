...
| [`universal_multiresolution_pitch.md`](universal_multiresolution_pitch.md) | **The learnable+lossless+universal pitch model** (supersedes per-tune-LUT / content-tier-residual). ONE shared canonical `2^(n/12)@C5` grid (proven: both driver tables are it ±1 LSB), decomposed into resolution passes: NOTE (semitone, structural, model predicts) + **per-voice** TUNING (fine, low-entropy; chorus detune is content — Cauldron II voices 1-2 ~12c/90%) + LSB (content tier, carried not predicted); modulation = idx-space trajectories. Per-tune-optimized fine resolution with a floor at the finest detune. Default-OFF, byte-exact + residual-zero gated. | **Active — validated (driver tables + corpus + chorus); implementation next** |
