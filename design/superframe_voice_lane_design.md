...
**Status:** Draft — **deferred behind the bare-melody-stream work** (the `no_unigram`
merge-isolation A/B and getting V0-onset off zero on the current stream come first).
...
1. **Now:** the `no_unigram` A/B isolates the merge weld on the *current* frame-major
   stream — get V0-onset off zero there first. See [`melody_learnability.md`](melody_learnability.md).
